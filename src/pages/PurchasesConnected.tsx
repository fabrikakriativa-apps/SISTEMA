...
type Purchase=PurchaseForConfirmation&{created_at:string;supplier:{name:string}|null;purchase_items:(PurchaseForConfirmation['purchase_items'][number]&{id:string})[]}
...
export function PurchasesConnected(){
 ...
 const load=useCallback(async()=>{if(!supabase||!access)return;setLoading(true);const [a,b,c]=await Promise.all([
 ...
 const statusLabel=(value:string)=>value==='draft'?'Rascunho':value==='awaiting_delivery'?'Aguardando entrega':value==='delayed'?'Atrasado':value==='completed'?'Recebido':value==='cancelled'?'Cancelado':value==='returned'?'Devolvido':value
 const statusOptions=(value:string)=>value==='draft'?['draft','awaiting_delivery','cancelled']:['awaiting_delivery','delayed'].includes(value)?[value,'completed','cancelled']:value==='completed'?['completed','returned']:[value]
 const openStatus=(purchase:Purchase,next:string)=>{if(next===purchase.status)return;setRequestedStatus(next);setDetail(purchase)}
 return <Page title="Compras" description="Necessidades dos pedidos, compras ao fornecedor e integração financeira.">
  ...
  {tab==='needs'?<section className="panel"><div className="toolbar"><span>{needs.length} item(ns) aguardando compra</span><select value={supplier} onChange={e=>setSupplier(e.target.value)}><option value="">Fornecedor depois</option>{suppliers.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select><select value={mode} onChange={e=>setMode(e.target.value as typeof mode)}><option value="made_to_order">Sob encomenda</option><option value="immediate">Compra imediata</option></select><button className="button primary" disabled={!selected.length||creating} onClick={()=>void create()}><ShoppingCart/>Criar pedido com {selected.length}</button></div>{loading?<p className="panel-message">Carregando…</p>:<div className="table-wrap"><table><thead><tr><th></th><th>Pedido / cliente</th><th>Tipo / ambiente</th><th>Descrição</th><th>Qtd.</th><th>Custo</th></tr></thead><tbody>{needs.map(x=><tr key={x.id} className={selected.includes(x.id)?'selected-row':''} onClick={()=>toggle(x)}><td><input type="checkbox" checked={selected.includes(x.id)} readOnly/></td>
...
<td><select className="status-select" aria-label={`Status de ${x.display_number}`} value={x.status} onClick={e=>e.stopPropagation()} onChange={e=>{e.stopPropagation();openStatus(x,e.target.value)}}>{statusOptions(x.status).map(status=><option key={status} value={status}>{statusLabel(status)}</option>)}</select></td>
