...
export function PurchasesConnected(){
 ...
 return <Page title="Compras" description="Necessidades dos pedidos, compras ao fornecedor e integração financeira.">
  ...
  {tab==='needs'?<section className="panel"><div className="toolbar"><span>{needs.length} item(ns) aguardando compra</span><select value={supplier} onChange={e=>setSupplier(e.target.value)}><option value="">Fornecedor depois</option>{suppliers.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select><select value={mode} onChange={e=>setMode(e.target.value as typeof mode)}><option value="made_to_order">Sob encomenda</option><option value="immediate">Compra imediata</option></select><button className="button primary" disabled={!selected.length||creating} onClick={()=>void create()}><ShoppingCart/>{creating?'Criando pedido…':selected.length?`Criar pedido (${selected.length} ${selected.length===1?'item selecionado':'itens selecionados'})`:'Selecione os itens'}</button></div>{loading?<p className="panel-message">Carregando…</p>:<div className="table-wrap"><table><thead><tr><th></th><th>Pedido / cliente</th><th>Tipo / ambiente</th><th>Descrição</th><th>Qtd.</th><th>Custo</th></tr></thead><tbody>{needs.map(x=><tr key={x.id} className={selected.includes(x.id)?'selected-row':''} onClick={()=>toggle(x)}><td><input type="checkbox" checked={selected.includes(x.id)} readOnly/></td>
