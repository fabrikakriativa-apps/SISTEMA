...
function BudgetEditor({access,budget,setBudget,form,setForm,clients,saveState,close}:{access:NonNullable<ReturnType<typeof useAccess>>;budget:Budget;setBudget:(budget:Budget)=>void;form:Editable;setForm:(value:Editable)=>void;clients:Client[];saveState:string;close:()=>void}) {
  ...
  const saveNewClient=async(event:FormEvent)=>{
    ...
    const payload={id:crypto.randomUUID(),organization_id:access.organizationId,client_type:'Cliente final',name:newClient.name.trim(),phone:newClient.phone.trim()||null,address:newClient.address.trim()||null,city:newClient.city.trim()||null,origin:newClient.origin||null,notes:newClient.notes.trim()||null,created_by:access.userId}
    const {data,error}=await supabase.from('clients').insert(payload).select('id,name,phone,address,city,client_type').single()
    if(error||!data)show('Não foi possível cadastrar o cliente. Confira os dados e tente novamente.','error')
    else {const saved=data as Client;setAvailableClients(current=>[...current,saved].sort((a,b)=>a.name.localeCompare(b.name,'pt-BR')));setClientSearch(saved.name);setForm({...form,client_id:saved.id,client_address:[saved.address,saved.city].filter(Boolean).join(' · '),client_address_edited:false});setNewClientOpen(false);setNewClient({name:'',phone:'',address:'',city:'',origin:'',notes:''});show('Cliente cadastrado e selecionado no orçamento.','success')}
  ...
  return <Page title="Construção do orçamento" description="Monte os dados comerciais e os itens que o cliente receberá." action={<div className="page-actions"><button className="button secondary" onClick={()=>setPreviewOpen(true)}>Prévia do cliente</button><button className="button secondary" onClick={close}><ArrowLeft/>Voltar aos orçamentos</button></div>}>
    ...
    {newClientOpen&&<div className="dialog-backdrop"><form className="dialog" onSubmit={saveNewClient}><header><div><span className="eyebrow">Cliente do orçamento</span><h2>Novo cliente</h2><p>O cadastro será incluído na base e selecionado neste orçamento.</p></div><button type="button" className="icon-button" onClick={()=>setNewClientOpen(false)}><X/></button></header><div className="form-grid"><label className="field span-2">Nome<input required value={newClient.name} onChange={e=>setNewClient({...newClient,name:e.target.value})}/></label><label className="field">Telefone<input value={newClient.phone} onChange={e=>setNewClient({...newClient,phone:e.target.value})}/></label><label className="field">Origem<select value={newClient.origin} onChange={e=>setNewClient({...newClient,origin:e.target.value})}><option value="">Não informada</option><option>Porta de loja</option><option>Instagram</option><option>Google</option><option>Indicação</option></select></label><label className="field span-2">Endereço<input value={newClient.address} onChange={e=>setNewClient({...newClient,address:e.target.value})} placeholder="Rua, número e complemento"/></label><label className="field">Cidade/UF<input value={newClient.city} onChange={e=>setNewClient({...newClient,city:e.target.value})}/></label><label className="field">Observação da origem<input value={newClient.notes} onChange={e=>setNewClient({...newClient,notes:e.target.value})}/></label></div><footer><button type="button" className="button secondary" onClick={()=>setNewClientOpen(false)}>Cancelar</button><button className="button primary" disabled={newClientSaving}>{newClientSaving?'Salvando…':'Salvar e selecionar'}</button></footer></form></div>}
