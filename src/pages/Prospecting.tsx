...
const blank:OpportunityForm = { id:'', name:'', phone:'', email:'', origin:'', notes:'', stage:'new', estimated_value:'', next_follow_up_at:'', lost_reason:'', client_id:null, budget_id:null }
...
export function Prospecting(){
  ...
  const [clients,setClients]=useState<ClientRef[]>([]), [search,setSearch]=useState(''), [open,setOpen]=useState(false), [form,setForm]=useState<OpportunityForm>(blank), [clientSearch,setClientSearch]=useState('')
  ...
  const edit=(item?:Opportunity)=>{
    ...
    const linked=item?.client_id?clients.find(client=>client.id===item.client_id):null
    setClientSearch(linked?clientOption(linked):'')
    setOpen(true)
  }
  const clientOption=(client:ClientRef)=>`${client.name} · ${client.phone||client.city||'sem telefone'}`
  const selectClient=(client:ClientRef)=>{setClientSearch(clientOption(client));setForm(current=>({...current,client_id:client.id}))}
  ...
  const saveAndCreateBudget=async()=>{
    if(!supabase||!access||saving||!form.id||form.stage==='lost')return
    setSaving(true)
    try{
      const validated=opportunityPayload({name:form.name,stage:form.stage,lost_reason:form.lost_reason})
      const payload={name:validated.name,phone:form.phone.trim()||null,email:form.email.trim()||null,origin:form.origin.trim()||null,notes:form.notes.trim()||null,stage:form.stage,estimated_value:form.estimated_value===''?null:form.estimated_value,next_follow_up_at:form.next_follow_up_at?new Date(form.next_follow_up_at).toISOString():null,lost_reason:validated.lost_reason,client_id:form.client_id||null}
      const {error:saveError}=await supabase.from('sales_opportunities').update(payload).eq('organization_id',access.organizationId).eq('id',form.id)
      if(saveError)throw saveError
      const {data,error}=await supabase.rpc('convert_opportunity_to_budget',{org_id:access.organizationId,target_opportunity_id:form.id})
      if(error)throw error
      show('Cliente vinculado e rascunho de orçamento criado.','success')
      setOpen(false); await load(); navigateTo('orcamentos',(data as {id:string}).id)
    }catch(reason){show(reason instanceof Error?reason.message:'Não foi possível criar o orçamento a partir da oportunidade.','error')}finally{setSaving(false)}
  }
  ...
  return <Page title="Prospecção" description="Contatos e parcerias B2B acompanhados até o orçamento." action={<button className="button primary" onClick={()=>edit()}><Plus/>Nova oportunidade</button>}>
